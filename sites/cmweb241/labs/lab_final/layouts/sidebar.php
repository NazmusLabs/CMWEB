<div class="sidebar"> 
  <!--🔍 Search Box - [DISABLED CODE]
			<script async src="https://cse.google.com/cse.js?cx=011282160671876409027:ptdgqqdcffz"></script>
      <div class="gcse-search"></div>
      <--/🔍 Search Box [DISABLED CODE]--> 
  <!--👆🏻 Sidebar Menu-->
  <nav class="menu-vertical" style="margin-right: 0">
    <h2>Labs Explorer</h2>
    <!--🔗 Menu Items-->
    <ul>
      <!--📜--> <!--📜--> 
      <!--🔗 Click!-->
      <li><a href="/sites/cmweb241/labs/lab1"><small>CMWEB 241 Lab 1</small></a></li>
      <!--🔗 Click!-->
      <li><a href="/sites/cmweb241/labs/lab2/"><small>CMWEB 241 Lab 2</small></a></li>
      <!--🔗 Click!-->
      <li><a href="/sites/cmweb241/labs/lab3/"><small>CMWEB 241 Lab 3</small></a></li>
      <!--🔗 Click!-->
      <li><a href="/sites/cmweb241/labs/lab4"><small>CMWEB 241 Lab 4</small></a></li>
      <!--🔗 Click!-->
      <li><a href="/sites/cmweb241/labs/lab5/"><small>CMWEB 241 Lab 5</small></a></li>
      <!--🔗 Click!-->
      <li><a href="/sites/cmweb241/labs/lab6/"><small>CMWEB 241 Lab 6</small></a></li>
      <!--🔗 Click!-->
      <li><a href="/sites/cmweb241/labs/lab7/"><small>CMWEB 241 Lab 7</small></a></li>
      <!--🔗 Click!-->
      <li><a href="/sites/cmweb241/labs/lab8/"><small>CMWEB 241 Lab 8</small></a></li>
      <!--🔗 Click!-->
      <li><a href="/sites/cmweb241/labs/lab9/"><small>CMWEB 241 Lab 9</small></a></li>
      <!--🔗 Click!-->
      <li><a href="/sites/cmweb241/labs/labx/"><small>CMWEB 241 Lab 10</small></a></li>
      <!--🔗 Click!-->
      <li><a href="/sites/cmweb241/labs/lab11/"><small>CMWEB 241 Lab 11</small></a></li>
      <!--🔗 Click!-->
      <li><a href="/sites/cmweb241/labs/lab12/"><small>CMWEB 241 Lab 12</small></a></li>
      <!--🔗🚫 Click!-->
      <li><small><a title="Click to reset this lab" style="color: black; text-decoration: none; cursor:default;" href="index.php">CMWEB 241 Lab 13 🍰 ✔</a></small></li>
      <!--📜--> <!--📜-->
    </ul>
    <!--/🔗 Menu Items-->
    <p> 
      <!--🔗 Click!--> <a class="button-ornate" href="/sites/cmweb241"> <span class="button-text-decoration">↵
      Course Home</span> </a> </p>
  </nav>
  <!--/👆🏻 Sidebar Menu--> 
</div>

<!--💡 Developer Remarks
=========================
The following meta tag is to let text editors, including Adobe Dreamweaver, know to open this file as a UTF-8 file and, therefore, prevent potential data loss.
--> 
<!--
<meta http-equiv="content-type" content="text/html; charset=UTF-8">
-->